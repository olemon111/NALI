...
#define USE_CLHT_CACHE
// #define CLHT_NUM_BUCKETS 1024 * 1024 // n*64B = 64MB
#define CLHT_NUM_BUCKETS 1024 * 1024 * 4 //  n*64B = 256MB
#define CACHE_SAMPLE 4
#endif
...
#ifdef USE_CACHE
            // if (!res || get_cnt % 32 == 0)
            if (get_cnt % CACHE_SAMPLE == 0)
            { // 采样
#ifdef USE_CLHT_CACHE
                clht_put(clht_cache_, key, payload);
#else
