...
static const char* pool_name = "/mnt/pmem1/zzy/template.data";
...
public:
    ...
    static bool Initialize(const char* pool_name, size_t pool_size){
        //if(pm_pool_ == nullptr){
        bool recover = false;
        if (!nali::FileExists(pool_name)) {
            LOG("creating a new pool");
            pm_pool_ = pmemobj_create_addr(pool_name, layout_name, pool_size,
                                            S_IWUSR | S_IRUSR, (void*)pool_addr);
            if (pm_pool_ == nullptr) {
                std::cout << "pool path: " << pool_name << std::endl;
