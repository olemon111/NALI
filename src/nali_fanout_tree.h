...
/*
 * This file contains utility code for using the fanout tree to help Nali
 * decide the best fanout and key partitioning scheme for Nali nodes
    ...
    for (int i = 0; i < level_fanout / 2; i++) {
      if (fanout_tree[level][2 * i].use && fanout_tree[level][2 * i + 1].use) {
        ...
        if (num_node_keys == 0) {
          ...
          best_cost -= kModelSizeWeight * sizeof(NaliDataNode<T, P>) *
                       total_keys / num_keys;
          continue;
        }
        ...
        double merging_cost_saving =
            ...
            (kModelSizeWeight * sizeof(NaliDataNode<T, P>) * total_keys /
             num_node_keys);
...
double compute_level(const std::pair<T, P> values[], int num_keys,
                     const NaliNode<T, P>* node, int total_keys,
  ...
  for (int i = 0; i < fanout; i++) {
    ...
    LinearModel<T> model;
    NaliDataNode<T, P>::build_model(values + left_boundary,
                                    right_boundary - left_boundary, &model,
                                    approximate_model_computation);

    DataNodeStats stats;
    double node_cost = NaliDataNode<T, P>::compute_expected_cost(
        values + left_boundary, right_boundary - left_boundary,
        NaliDataNode<T, P>::kInitDensity_, expected_insert_frac, &model,
  ...
  double traversal_cost =
      ...
      (kModelSizeWeight * fanout *
       (sizeof(NaliDataNode<T, P>) + sizeof(void*)) * total_keys / num_keys);
  cost += traversal_cost;
  return cost;
}
...
std::pair<int, double> find_best_fanout_bottom_up(
    const std::pair<T, P> values[], int num_keys, const NaliNode<T, P>* node,
  ...
  while (true) {
    ...
    for (FTNode& tree_node : fanout_tree[fanout_tree_level - 1]) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        NaliDataNode<T, P>::build_model(values + left, right - left,
                                        &node_models[i],
                                        approximate_model_computation);
        node_costs[i] = NaliDataNode<T, P>::compute_expected_cost(
            values + left, right - left, NaliDataNode<T, P>::kInitDensity_,
            expected_insert_frac, &node_models[i], approximate_cost_computation,
            &node_stats[i]);
      }
      node_split_cost += sizeof(NaliDataNode<T, P>) * kModelSizeWeight *
...
template <class T, class P>
int find_best_fanout_existing_node(const NaliModelNode<T, P>* parent,
                                   int bucketID, int total_keys,
                                   std::vector<FTNode>& used_fanout_tree_nodes,
                                   int max_fanout) {
  // Repeatedly add levels to the fanout tree until the overall cost of each
  // level starts to increase
  auto node = static_cast<NaliDataNode<T, P>*>(parent->children_[bucketID]);
  ...
  for (int fanout = 1, fanout_tree_level = 0; fanout <= max_fanout;
    ...
    for (int i = 0; i < fanout; i++) {
      ...
      LinearModel<T> model;
      typename NaliDataNode<T, P>::const_iterator_type it(node, left_boundary);
      ...
      DataNodeStats stats;
      double node_cost =
          NaliDataNode<T, P>::compute_expected_cost_from_existing(
              node, left_boundary, right_boundary,
              NaliDataNode<T, P>::kInitDensity_, empirical_insert_frac, &model,
              &stats);
