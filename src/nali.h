...
namespace nali
{
    ...
    void Nali::bulk_load(std::vector<std::pair<uint64_t, uint64_t>> &data)
    {
    ...
    void Nali::bulk_load(const std::pair<uint64_t, uint64_t> data[], int size)
    {
        int group_id = 0;
    ...
    status Nali::Put(uint64_t key, uint64_t value)
    {
    ...
    bool Nali::Get(uint64_t key, uint64_t &value)
    {
#ifdef MULTI_THREAD
        trans_begin();
#endif
    ...
    int Nali::find_group(const uint64_t &key) const
    {
    ...
    bool Nali::find_fast(uint64_t key, uint64_t &value) const
    {
        int group_id = model.predict(key) / min_entry_count;
        group_id = std::min(std::max(0, group_id), (int)nr_groups_ - 1);
#ifdef MULTI_THREAD
    ...
    bool Nali::find_slow(uint64_t key, uint64_t &value) const
    {
        int group_id = find_group(key);
#ifdef MULTI_THREAD
        // std::unique_lock<std::mutex> lock(lock_space[group_id]);
    ...
    bool Nali::scan_fast(uint64_t start_key, int &len, std::vector<std::pair<uint64_t, uint64_t>> &results) const
    {
    ...
    bool Nali::scan_slow(uint64_t start_key, int &len, std::vector<std::pair<uint64_t, uint64_t>> &results, int fast_group_id) const
    {
        int group_id = 0;
        if (fast_group_id != 0)
        {
