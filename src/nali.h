  ...
  private:
    ...
    void freeForEpoch(uint32_t epoch) {
      ...
      for (void *pointer : previousFreeList) {
        auto node = reinterpret_cast<NaliNode<T, P> *>(pointer);
        if (node == nullptr) {
          return;
...
private:
  ...
  void safe_delete_node(NaliNode<T, P> *node) {
...
public:
  ...
  inline bool lock_and_correct_traversal_path(
      ...
      bool left, NaliNode<T, P> *child_node, model_node_type **locked_node,
  ...
  forceinline bool lock_parent_node(T key,
    ...
    if (cur == child_node) {
      if (superroot_->get_read_lock()) {
        ...
        if ((child_node == static_cast<NaliNode<T, P> *>(root_node_)) &&
