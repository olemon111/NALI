...
#include "nali_base.h"
#include "nali_fanout_tree.h"
#include "nali_nodes.h"
...
class Nali {
  static_assert(std::is_arithmetic<T>::value, "Nali key type must be numeric.");
  static_assert(std::is_same<Compare, NaliCompare>::value,
                "Must use NaliCompare.");
 ...
 public:
  ...
  // Nali class aliases
  typedef Nali<T, P, Compare, Alloc, allow_duplicates> self_type;
  typedef NaliModelNode<T, P, Alloc> model_node_type;
  ...
  struct Params {
    // When bulk loading, Nali can use provided knowledge of the expected
 ...
 public:
  ...
  Nali(const Compare& comp, const Alloc& alloc = Alloc())
  ...
  Nali(const Alloc& alloc) : allocator_(alloc) {
    // Set up root as empty data node
    auto empty_data_node = new (data_node_allocator().allocate(1))
        data_node_type(key_less_, allocator_);
    empty_data_node->bulk_load(nullptr, 0);
 ...
 public:
  // When bulk loading, Nali can use provided knowledge of the expected fraction
...
// node's parent.
 ...
 private:
  ...
  inline void correct_traversal_path(data_node_type* leaf,
    ...
    if (left) {
      ...
      if (start_bucketID == 0) {
        ...
        NaliNode<T, P>* cur = parent->children_[correct_bucketID];
        while (!cur->is_leaf_) {
          auto node = static_cast<model_node_type*>(cur);
  ...
  void link_all_data_nodes() {
    ...
    for (NodeIterator node_it = NodeIterator(this); !node_it.is_end();
         node_it.next()) {
      NaliNode<T, P>* cur = node_it.current();
      if (cur->is_leaf_) {
 ...
 private:
  ...
  void delete_node(NaliNode<T, P>* node) {
    if (node == nullptr) {
      return;
 ...
 private:
  ...
  void create_superroot() {
    ...
    superroot_->children_ =
        new (pointer_allocator().allocate(1)) NaliNode<T, P>*[1];
  ...
  // dense keys.
  void bulk_load_node(const V values[], int num_keys, NaliNode<T, P>*& node,
                      int total_keys,
    ...
    if (best_fanout_tree_cost < node->cost_ ||
      ...
      model_node->children_ =
          new (pointer_allocator().allocate(fanout)) NaliNode<T, P>*[fanout];
 ...
 private:
  ...
  int best_split_propagation(const std::vector<TraversalNode>& traversal_path,
    ...
    for (const TraversalNode& tn : traversal_path) {
      double stop_cost;
      NaliNode<T, P>* next = tn.node->children_[tn.bucketID];
 ...
 public:
  ...
  // This is just a function required by the STL standard. Nali can hold more
  // items.
 ...
 public:
  ...
  bool validate_structure(bool validate_data_nodes = false,
    ...
    bool is_valid = true;
    std::stack<NaliNode<T, P>*> node_stack;
    NaliNode<T, P>* cur;
 ...
 public:
  ...
  class NodeIterator {
   public:
    ...
    NaliNode<T, P>* cur_node_;
    std::stack<NaliNode<T, P>*> node_stack_;  // helps with traversal
    ...
    NaliNode<T, P>* current() const { return cur_node_; }
