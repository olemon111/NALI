...
/*
 * This file contains code for Nali nodes. There are two types of nodes in Nali:
...
#define Nali_DATA_NODE_KEY_AT(i) key_slots_[i]
#define Nali_DATA_NODE_PAYLOAD_AT(i) payload_slots_[i]
#else
#define Nali_DATA_NODE_KEY_AT(i) data_slots_[i].first
#define Nali_DATA_NODE_PAYLOAD_AT(i) data_slots_[i].second
#endif
...
#define Nali_USE_LZCNT 1

namespace nali {

// A parent class for both types of Nali nodes
...
class NaliNode {
 public:
  ...
  NaliNode() = default;
  explicit NaliNode(short level) : level_(level) {}
  NaliNode(short level, bool is_leaf) : is_leaf_(is_leaf), level_(level) {}
  virtual ~NaliNode() = default;
...
class NaliModelNode : public NaliNode<T, P> {
 public:
  typedef NaliModelNode<T, P, Alloc> self_type;
  typedef typename Alloc::template rebind<self_type>::other alloc_type;
  typedef typename Alloc::template rebind<NaliNode<T, P>*>::other
      pointer_alloc_type;
  ...
  NaliNode<T, P>** children_ = nullptr;

  explicit NaliModelNode(const Alloc& alloc = Alloc())
      : NaliNode<T, P>(0, false), allocator_(alloc) {}

  explicit NaliModelNode(short level, const Alloc& alloc = Alloc())
      : NaliNode<T, P>(level, false), allocator_(alloc) {}
  ...
  inline NaliNode<T, P>* get_child_node(const T& key) {
  ...
  int expand(int log2_expansion_factor) {
    ...
    auto new_children = new (pointer_allocator().allocate(num_new_children))
        NaliNode<T, P>*[num_new_children];
    int cur = 0;
    while (cur < num_children_) {
      NaliNode<T, P>* cur_child = children_[cur];
  ...
  long long node_size() const override {
    ...
    size += num_children_ * sizeof(NaliNode<T, P>*);  // pointers to children
    return size;
  }
  ...
  bool validate_structure(bool verbose = false) const {
    ...
    NaliNode<T, P>* cur_child = children_[0];
    int cur_repeats = 1;
    int i;
...
* - Debugging
*/
template <class T, class P, class Compare = NaliCompare,
          class Alloc = std::allocator<std::pair<T, P>>,
          bool allow_duplicates = true>
class NaliDataNode : public NaliNode<T, P> {
 public:
  typedef std::pair<T, P> V;
  typedef NaliDataNode<T, P, Compare, Alloc, allow_duplicates> self_type;
  typedef typename Alloc::template rebind<self_type>::other alloc_type;
  ...
  explicit NaliDataNode(const Compare& comp = Compare(),
                        const Alloc& alloc = Alloc())
      : NaliNode<T, P>(0, true), key_less_(comp), allocator_(alloc) {}

  NaliDataNode(short level, int max_data_node_slots,
               const Compare& comp = Compare(), const Alloc& alloc = Alloc())
      : NaliNode<T, P>(level, true),
        key_less_(comp),
        allocator_(alloc),
  ...
  NaliDataNode(const self_type& other)
      : NaliNode<T, P>(other),
  ...
  inline T& get_key(int pos) const { return Nali_DATA_NODE_KEY_AT(pos); }

  inline P& get_payload(int pos) const {
    return Nali_DATA_NODE_PAYLOAD_AT(pos);
  }
  ...
  void bulk_load(const V values[], int num_keys,
    ...
    if (num_keys == 0) {
      ...
      for (int i = 0; i < data_capacity_; i++) {
        Nali_DATA_NODE_KEY_AT(i) = kEndSentinel_;
      }
      return;
    }
    ...
    for (int i = 0; i < num_keys; i++) {
      ...
      if (positions_remaining < keys_remaining) {
        ...
        for (int j = last_position + 1; j < pos; j++) {
          Nali_DATA_NODE_KEY_AT(j) = values[i].first;
        }
        for (int j = i; j < num_keys; j++) {
#if Nali_DATA_NODE_SEP_ARRAYS
  ...
  void bulk_load_from_existing(
    ...
    for (; it.cur_idx_ < right && !it.is_end(); it++) {
      ...
      if (positions_remaining < keys_remaining) {
        ...
        int pos = data_capacity_ - keys_remaining;
        for (int j = last_position + 1; j < pos; j++) {
          Nali_DATA_NODE_KEY_AT(j) = it.key();
        }
    ...
    max_key_ = Nali_DATA_NODE_KEY_AT(last_position);
  ...
  int find_key(const T& key) {
    ...
    if (pos < 0 || !key_equal(Nali_DATA_NODE_KEY_AT(pos), key)) {
      return -1;
    } else {
      return pos;
    }
  ...
  inline int exponential_search_upper_bound(int m, const K& key) {
    ...
    if (key_greater(Nali_DATA_NODE_KEY_AT(m), key)) {
      int size = m;
      while (bound < size &&
             key_greater(Nali_DATA_NODE_KEY_AT(m - bound), key)) {
        bound *= 2;
      ...
      while (bound < size &&
             key_lessequal(Nali_DATA_NODE_KEY_AT(m + bound), key)) {
        bound *= 2;
        num_exp_search_iterations_++;
      }
      l = m + bound / 2;
  ...
  inline int binary_search_upper_bound(int l, int r, const K& key) const {
    while (l < r) {
      int mid = l + (r - l) / 2;
      if (key_lessequal(Nali_DATA_NODE_KEY_AT(mid), key)) {
        l = mid + 1;
      } else {
        r = mid;
      }
  ...
  inline int exponential_search_lower_bound(int m, const K& key) {
    ...
    int bound = 1;
    int l, r;  // will do binary search in range [l, r)
    if (key_greaterequal(Nali_DATA_NODE_KEY_AT(m), key)) {
      int size = m;
      while (bound < size &&
             key_greaterequal(Nali_DATA_NODE_KEY_AT(m - bound), key)) {
        bound *= 2;
        num_exp_search_iterations_++;
      }
      l = m - std::min<int>(bound, size);
      ...
      int size = data_capacity_ - m;
      while (bound < size && key_less(Nali_DATA_NODE_KEY_AT(m + bound), key)) {
        bound *= 2;
        num_exp_search_iterations_++;
      }
      l = m + bound / 2;
  ...
  inline int binary_search_lower_bound(int l, int r, const K& key) const {
    while (l < r) {
      int mid = l + (r - l) / 2;
      if (key_greaterequal(Nali_DATA_NODE_KEY_AT(mid), key)) {
        r = mid;
      } else {
        l = mid + 1;
      }
  ...
  std::pair<int, int> insert(const T& key, const P& payload) {
    ...
    if (!allow_duplicates && upper_bound_pos > 0 &&
        key_equal(Nali_DATA_NODE_KEY_AT(upper_bound_pos - 1), key)) {
  ...
  int erase_one(const T& key) {
    ...
    if (pos == data_capacity_ || !key_equal(Nali_DATA_NODE_KEY_AT(pos), key))
      return 0;
  ...
  void erase_one_at(int pos) {
    ...
    if (pos == data_capacity_ - 1) {
      ...
      next_key = Nali_DATA_NODE_KEY_AT(pos + 1);
    }
    Nali_DATA_NODE_KEY_AT(pos) = next_key;
    unset_bit(pos);
    pos--;
  ...
  int erase(const T& key) {
    ...
    if (pos == 0 || !key_equal(Nali_DATA_NODE_KEY_AT(pos - 1), key)) return 0;
    ...
    if (pos == data_capacity_) {
      ...
      next_key = Nali_DATA_NODE_KEY_AT(pos);
    }
    pos--;
    while (pos >= 0 && key_equal(Nali_DATA_NODE_KEY_AT(pos), key)) {
      Nali_DATA_NODE_KEY_AT(pos) = next_key;
  ...
  int erase_range(T start_key, T end_key, bool end_key_inclusive = false) {
    ...
    pos--;
    while (pos >= 0 &&
           key_greaterequal(Nali_DATA_NODE_KEY_AT(pos), start_key)) {
      Nali_DATA_NODE_KEY_AT(pos) = next_key;
      num_erased += check_exists(pos);
      unset_bit(pos);
      pos--;
  ...
  bool validate_structure(bool verbose = false) const {
    ...
    for (int i = 0; i < data_capacity_ - 1; i++) {
      if (key_greater(Nali_DATA_NODE_KEY_AT(i), Nali_DATA_NODE_KEY_AT(i + 1))) {
        if (verbose) {
          std::cout << "Keys should be in non-increasing order" << std::endl;
        }
        return false;
      } else if (key_less(Nali_DATA_NODE_KEY_AT(i),
                          Nali_DATA_NODE_KEY_AT(i + 1)) &&
                 !check_exists(i)) {
        if (verbose) {
    ...
    if (Nali_DATA_NODE_KEY_AT(data_capacity_ - 1) == kEndSentinel_ &&
  ...
  bool key_exists(const T& key, bool validate_bitmap) const {
    for (int i = 0; i < data_capacity_ - 1; i++) {
      if (key_equal(Nali_DATA_NODE_KEY_AT(i), key) &&
  ...
  std::string to_string() const {
    ...
    for (int i = 0; i < data_capacity_; i++) {
      str += (std::to_string(Nali_DATA_NODE_KEY_AT(i)) + " ");
    }
    return str;
  }
};
