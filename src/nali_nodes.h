...
template <class T, class P> class NaliNode {
public:
...
public:
  ...
  NaliModelNode(const self_type &other)
      : NaliNode<T, P>(other), allocator_(other.allocator_),
  ...
  */
  ...
  void expand(NaliModelNode<T, P> *old_node, int log2_expansion_factor) {
...
public:
  ...
  double max_limit_ = 0; // zzy TODO， nali对初始节点没有做这个
  double min_limit_ = 0;
  ...
  NaliDataNode(short level, int max_data_node_slots,
      ...
      : NaliNode<T, P>(level, true), key_less_(comp), allocator_(alloc),
  ...
  NaliDataNode(const self_type &other)
      : NaliNode<T, P>(other), key_less_(other.key_less_),
