...
function Run() {
    ...
    Loadname="ycsb-200m"
    loadnum=190000000
    date | tee multi-${dbname}-${Loadname}-th${thread}-s${valuesize}-b${bgthreads}.txt
    # gdb --args \
    ...
    timeout 1200 ${BUILDDIR}nali_multi_bench --dbname ${dbname} \
        ...
        --thread-nums $thread --bgthreads $bgthreads | tee -a multi-${dbname}-${Loadname}-th${thread}-s${valuesize}-b${bgthreads}.txt
    echo "----------------"
    sleep 5
    ...
    for dbname in $dbs; do
        ...
        do
            ...
            do
                for thread in 16
                do
                    Run $dbname $loadnum $opnum $scansize $thread $valsize $bgthreads
                done
            done
        done
    done
