    ...
    Pointer8B pfirst[], int n_nodes[])
{
    ...
    n_nodes[0] = lbtree_ceiling(num_key, leaf_fill_num);
    top_level = 0;

    for (int i = 1; n_nodes[i - 1] > 1 && i <= target_level; i++)
    {
        n_nodes[i] = lbtree_ceiling(n_nodes[i - 1], nonleaf_fill_num + 1);
        top_level = i;
...
int lbtree::bulkload(int keynum, keyInput *input, float bfill)
{
    ...
    int kn_per_thread = lbtree_floor(keynum, num_threads);
    ...
    //    using the lbtree_min top level
...
/**
 * get lbtree_min and lbtree_max key in the given leaf p
 */
