...
static inline void clwbmore(void *start, void *end)
{
  ...
  do
  {
    ...
    start_line += LBTREE_CACHE_LINE_SIZE;
...
static inline void clwbmore(void *start, void *end)
{
  unsigned long long start_line = getline(start);
  unsigned long long end_line = getline(end);
  num_clwb += (end_line + LBTREE_CACHE_LINE_SIZE - start_line) / LBTREE_CACHE_LINE_SIZE;
 ...
 *
 * +-----------+--------+------------+-----+--------------------------+
 * | flag byte | offset |log byte[2] | ... | log byte[LBTREE_CACHE_LINE_SIZE-1] |
 * +-----------+--------+------------+-----+--------------------------+
 *
...
public:
  ...
  void prepareLogforWriting()
  {
    ...
    nl_log_wr_.nextline_ptr_ = nl_log_area_ + LBTREE_CACHE_LINE_SIZE;
  ...
  void printLog()
  {
    ...
    {
      if ((int)(p - nl_log_area_) % LBTREE_CACHE_LINE_SIZE == 0)
      {
  ...
  void printLogWritePos()
  {
    printf("write version_:%02x, offset_:%3d, next_ptr_:%d, nextline_ptr_:%d\n",
           nl_log_wr_.version_,
           nl_log_wr_.offset_,
           (int)(nl_log_wr_.next_ptr_ - (nl_log_wr_.nextline_ptr_ - LBTREE_CACHE_LINE_SIZE)),
...
private:
  ...
  {
    char *line = nl_log_wr_.nextline_ptr_ - LBTREE_CACHE_LINE_SIZE;
    ...
    if (nl_log_wr_.nextline_ptr_ < nl_log_area_end_)
    {
      ...
      nl_log_wr_.nextline_ptr_ += LBTREE_CACHE_LINE_SIZE;
    }
    else
    {
...
public:
  ...
  void writeLog(const char *rec, int len)
  {
    ...
    {
      nl_log_wr_.offset_ = LBTREE_CACHE_LINE_SIZE - l;
    }
  ...
  int readLog(nl_log_pointer_t *pos, char *buf, int len)
  {
    ...
    while ((len > 0) && checkByteOne(pos->nextline_ptr_ - LBTREE_CACHE_LINE_SIZE, pos->version_))
    {
      ...
      if (pos->next_ptr_ == pos->nextline_ptr_)
      {
        ...
        {
          pos->next_ptr_ += 2;
          pos->nextline_ptr_ += LBTREE_CACHE_LINE_SIZE;
        }
        else
        {
          pos->version_ ^= 0x80;
          pos->next_ptr_ = nl_log_area_ + 2;
          pos->nextline_ptr_ = nl_log_area_ + LBTREE_CACHE_LINE_SIZE;
        }
    ...
    while ((len > 0) && checkByteOne(pos->nextline_ptr_, pos->version_))
    {
      ...
      if (pos->next_ptr_ - 2 == pos->nextline_ptr_)
      {
        ...
        else
        {
          ...
          pos->nextline_ptr_ = nl_log_area_end_ - LBTREE_CACHE_LINE_SIZE;
        }
  ...
  {
    if ((((unsigned long long)(ptr)) & (LBTREE_CACHE_LINE_SIZE - 1)) <= 2)
    {
