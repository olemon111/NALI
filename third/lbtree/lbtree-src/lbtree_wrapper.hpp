...
float bfill = 1.0;
class lbtree_wrapper
{
public:
  lbtree_wrapper(void *nvm_addr, bool recover);
  virtual ~lbtree_wrapper();

  virtual bool find(key_type key, uint64_t &value);
  virtual bool insert(key_type key, const uint64_t &value);
  virtual bool update(key_type key, const uint64_t &value);
  virtual bool remove(key_type key);
  virtual int scan(key_type key, uint32_t to_scan, std::pair<key_type, uint64_t>*& values_out);
 ...
 */
struct LbtreeThreadHelper
{
  const char *str;
  int id_ = -69;
  LbtreeThreadHelper(const char *s) : str{s}
  {
    if (worker_id < 0)
...
bool lbtree_wrapper::find(key_type key, uint64_t &value) {
  thread_local LbtreeThreadHelper t{FIND};
  void *p;
  int pos = -1;
#ifdef VAR_KEY
...
bool lbtree_wrapper::insert(key_type key, const uint64_t &value) {
  thread_local LbtreeThreadHelper t{INSERT};
...
bool lbtree_wrapper::update(key_type key, const uint64_t &value) {
thread_local LbtreeThreadHelper T{UPDATE};
  bnode *p;
  bleaf *lp;
  int i, t, m, b, jj;
  auto k = key;
...
bool lbtree_wrapper::remove(key_type key) {
  thread_local LbtreeThreadHelper t{REMOVE};
  //FIXME
  lbt->del(key);
...
int lbtree_wrapper::scan(key_type key, uint32_t to_scan, std::pair<key_type, uint64_t>*& values_out) {
  thread_local LbtreeThreadHelper t{SCAN};
  constexpr size_t ONE_MB = 1ULL << 20;
  static thread_local char results[ONE_MB];
  // FIXME
  // TODO: modify value
  // values_out = results;
  int scanned = lbt->rangeScan(key, to_scan, results); // range_scan_by_size
#ifdef DEBUG_MSG
  if (scanned != 100)
...
extern lbtree_wrapper *create_lbtree(int num_threads);