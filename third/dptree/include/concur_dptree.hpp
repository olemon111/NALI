...
template <typename DPKey, typename Value>
...
public:
    ...
    struct alignas(64) leaf_node
    {
            ...
            int &split_merges)
        {
            ...
            while (i < l_key_count && upsert_kvs_sit != upsert_kvs_eit)
            {
                ...
                #ifdef VAR_KEY
                    // free_idx =
                    //     node_alloc_bitmap->alloc_first_unset_bit(std::_Hash_bytes((char*)upsert_key, dptree_key_size_, 1));
                    free_idx =
                ...
                #ifdef VAR_KEY
                    // free_idx =
                    //     node_alloc_bitmap->alloc_first_unset_bit(std::_Hash_bytes((char*)cv_key, dptree_key_size_, 1));
            ...
            while (i < l_key_count)
            {
                ...
                // int free_idx =
                //     node_alloc_bitmap->alloc_first_unset_bit(std::_Hash_bytes((char*)l->pairs[cv_key_idx].first, dptree_key_size_, 1));
            ...
            while (upsert_kvs_sit != upsert_kvs_eit)
            {
                ...
                // int free_idx =
                //     node_alloc_bitmap->alloc_first_unset_bit(std::_Hash_bytes((char*)upsert_kvs_sit->first, dptree_key_size_, 1));
                int free_idx = node_alloc_bitmap->alloc_first_unset_bit(
            ...
            int &split_merges)
        {
            ...
            while (i < l_key_count && upsert_kvs_sit != upsert_kvs_eit)
            {
                ...
                {
                #ifdef VAR_KEY
                    // free_idx = node_alloc_bitmap->alloc_first_unset_bit(std::_Hash_bytes((char*)cv_key, dptree_key_size_, 1));
                    free_idx = node_alloc_bitmap->alloc_first_unset_bit(hasher(*(uint64_t*)cv_key));
                #else
                    free_idx = node_alloc_bitmap->alloc_first_unset_bit(hasher(cv_key));
                #endif
            ...
            while (i < l_key_count)
            {
                ...
                assert(l->pairs[cv_key_idx].first != 0 && "l->pairs[cv_key_idx].first is 0!!?\n");
                // int free_idx = node_alloc_bitmap->alloc_first_unset_bit(
                //     std::_Hash_bytes((char*)l->pairs[cv_key_idx].first, dptree_key_size_, 1));
                int free_idx = node_alloc_bitmap->alloc_first_unset_bit(
                    hasher(*(uint64_t*)l->pairs[cv_key_idx].first));
            #else
                int free_idx = node_alloc_bitmap->alloc_first_unset_bit(
        ...
        preserve_l:
            ...
            while (upsert_kvs_sit != upsert_kvs_eit)
            {
                ...
                #ifdef VAR_KEY
                    // int free_idx =
                    //     l_alloc_bitmap->alloc_first_unset_bit(std::_Hash_bytes((char*)upsert_key, dptree_key_size_, 1));
                    int free_idx =
                    ...
                    int &update_inplace, int &split_merges, int &perserve_l)
        {
            ...
            while (upsert_kvs_sit != upsert_kvs_eit)
            {
                ...
                assert(upsert_kvs_sit->first != 0 && "upsert_kvs_sit->first is 0???\n");
                // int free_idx = node_alloc_bitmap->alloc_first_unset_bit(
                //     std::_Hash_bytes((char*)upsert_kvs_sit->first, dptree_key_size_, 1));
                int free_idx = node_alloc_bitmap->alloc_first_unset_bit(
                    hasher(*(uint64_t*)upsert_kvs_sit->first));
            #else
                int free_idx = node_alloc_bitmap->alloc_first_unset_bit(
                             ...
                             bool nv)
        {
            ...
            for (int i = 0; i < leafs.size(); ++i)
            {
                ...
                for (int j = 0; j < l_key_count; ++j)
                {
                    ...
                    // int free_idx = node_alloc_bitmap->alloc_first_unset_bit(
                    //     std::_Hash_bytes((char*)l->ith_key(j, nv), dptree_key_size_, 1));
                    int free_idx = node_alloc_bitmap->alloc_first_unset_bit(
                                 ...
                                 leaf_node *end_leaf = nullptr)
        {
            ...
            { // l1's keys fit in l2's free cells
                ...
                {
                #ifdef VAR_KEY
                    // int free_idx = l2_alloc_bitmap->alloc_first_unset_bit(std::_Hash_bytes((char*)l1->ith_key(i, nv), dptree_key_size_, 1));
    ...
    leaf_node *lower_bound_leaf(const key_type &key)
    {
        ...
        uint8_t lookup_key[dptree_key_size_];
        // *reinterpret_cast<uint64_t *>(lookup_key) = __builtin_bswap64(*(uint64_t*)key);
        memcpy(lookup_key, (char*)key, dptree_key_size_);
        auto leaf = tree->lowerBound(lookup_key, dptree_key_size_, 0,
                                     dptree_key_size_, pref);
    #else
                        ...
                        int nworkers = 1)
    {
        ...
        #ifdef VAR_KEY
            uint64_t max_key = (uint64_t) new char[dptree_key_size_];
            memset((char*)max_key, 0, dptree_key_size_);
        ...
        // 6. build in-memory radix-tree index for leaf nodes
        {
            ...
            for (size_t i = 0; i < nworkers; ++i)
            {
                for (auto p : merge_worker_stats[i].maxkey_node_pairs)
                {
                    ...
                    // assert(key != 0 && "key is 0!\n");
                    art_trees[nv]->insert((uint8_t *)key, value, dptree_key_size_);
                #else
...
//                   stat.flushed_node, stat.flushes, stat.update_inplace, stat.split_merges, stat.perserve_l, stat.node_merges_transfer, stat.node_merges_combine, stats[nv].avg_fill());
    ...
    bool lookup(const key_type &key, value_type &value)
    {
        ...
        *reinterpret_cast<uint64_t *>(lookup_key) = __builtin_bswap64(key);
        ...
        auto cur = reinterpret_cast<leaf_node *>(ART_IDX::getLeafValue(leaf));
    #ifdef VAR_KEY
        size_t h = hasher(*(uint64_t*)key); // std::_Hash_bytes((char*)key, dptree_key_size_, 1);
    #else
        size_t h = hasher(key);
    #endif
...
template <class DPKey, class Value>
struct log_record
{
    using key_type = DPKey;
    using value_type = Value;
...
template <class DPKey, class Value, int PageSize = 64 * 1024>
class lockfree_pwal
{
  public:
    using key_type = DPKey;
...
class durable_concur_buffer_btree
{
  public:
    ...
    void insert(const key_type &key, const value_type &value)
    {
        ...
        #ifdef VAR_KEY
            int id = std::hash<key_type>()(*(uint64_t*)key) % stripes;// std::_Hash_bytes((char*)key, dptree_key_size_, 1) % stripes;
        #else
...
template <class DPKey, class Value>
thread_local log_record<DPKey, Value> *durable_concur_buffer_btree<DPKey, Value>::local_record = nullptr;

template <class DPKey, class Value>
thread_local log_record<DPKey, Value> *durable_concur_buffer_art<DPKey, Value>::local_record = nullptr;
...
template <class DPKey, class Value>
thread_local thread_epoch concur_dptree<DPKey, Value>::front_writer_local_epoch;

template <class DPKey, class Value>
thread_local thread_epoch concur_dptree<DPKey, Value>::front_reader_local_epoch;

template <class DPKey, class Value>
thread_local thread_epoch concur_dptree<DPKey, Value>::middle_reader_local_epoch;

template <class DPKey, class Value>
thread_local thread_epoch concur_cvhtree<DPKey, Value>::reader_epoch;
