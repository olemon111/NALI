...
*/
    ...
    auto start = rdtsc();
#endif
    auto res = memcmp(a, b, dptree_key_size_);
#ifdef PROFILE
...
template <class DPKey, class Payload>
struct BTreeLeaf : public BTreeLeafBase
{
    // This is the element type of the leaf node
    using KeyValueType = std::pair<DPKey, Payload>;
    ...
    struct BTreeLeaf<DPKey, Payload> *next;
    BTreeLeaf()
    {
        count = 0;
        type = typeMarker;
    ...
    unsigned lowerBound(DPKey k)
    {
        ...
        //     const DPKey &middle_key = data[mid].first;
    ...
    void update(DPKey k, Payload p, unsigned pos) {
    ...
    void insert_at(DPKey k, Payload p, unsigned pos) {
        assert(data[pos].first == k);
        if (count) {
...
template <class DPKey>
struct BTreeInner : public BTreeInnerBase
{
    static const uint64_t maxEntries =
        (pageSize - sizeof(NodeBase)) / (sizeof(DPKey) + sizeof(NodeBase *));
    DPKey keys[maxEntries];
    ...
    void insert(DPKey k, NodeBase *child)
    {
        assert(count < maxEntries - 1);
        unsigned pos = lowerBound(k);
        memmove(keys + pos + 1, keys + pos, sizeof(DPKey) * (count - pos + 1));
...
struct BTree
{
    ...
    BTree() { root = new BTreeLeaf<DPKey, Value>(); }

    void makeRoot(DPKey k, NodeBase *leftChild, NodeBase *rightChild)
    {
        auto inner = new BTreeInner<DPKey>();
        inner->count = 1;
        inner->keys[0] = k;
    ...
    void destroy(NodeBase* node) {
        ...
        if (node->type == PageType::BTreeInner) {
            auto inner = static_cast<BTreeInner<DPKey> *>(node);
            for (int i = 0; i < inner->count; ++i) {
                destroy(inner->children[i]);
            }
            delete inner;
        } else {
            auto leaf = static_cast<BTreeLeaf<DPKey, Value> *>(node);
            delete leaf;
        }
    }
    ~BTree() {
    ...
    void insert(DPKey k, Value v, std::function<void()> insert_func=noop)
    {
        int restartCount = 0;
    restart:
        if (restartCount++)
        ...
        BTreeInner<DPKey> *parent = nullptr;
        ...
        while (node->type == PageType::BTreeInner)
        {
            ...
            if (inner->isFull())
            {
                ...
                // Split
                DPKey sep;
                BTreeInner<DPKey> *newInner = inner->split(sep);
                if (parent)
        ...
        if (leaf->count == leaf->maxEntries)
        {
            ...
            // Split
            DPKey sep;
            BTreeLeaf<DPKey, Value> *newLeaf = leaf->split(sep);
        #ifdef VAR_KEY
            if (vkcmp((char*)k, (char*)sep) > 0)
        #else
            if (k > sep) 
    ...
    void upsert(DPKey k, Value v, std::function<bool(DPKey k)> should_insert_func, std::function<void()> insert_func = noop)
    {
        int restartCount = 0;
        restart:
        if (restartCount++)
    ...
    bool lookup(DPKey k, Value &result)
    {
        int restartCount = 0;
    restart:
        if (restartCount++)
        ...
        BTreeLeaf<DPKey, Value> *leaf = static_cast<BTreeLeaf<DPKey, Value> *>(node);
        unsigned pos = leaf->lowerBound(k);
        bool success = false;
    #ifdef VAR_KEY
    ...
    uint64_t scan(DPKey k, int range, typename BTreeLeaf<DPKey, Value>::KeyValueType *output)
    {
        int restartCount = 0;
    restart:
        if (restartCount++)
    ...
    {
        BTree<DPKey, Value> *btree;
        int count;
        int pos;
        static const uint64_t scan_unit_size = BTreeLeaf<DPKey, Value>::maxEntries;
        typename BTreeLeaf<DPKey, Value>::KeyValueType kvs[scan_unit_size];

        explicit range_iterator(BTree<DPKey, Value> *btree, const DPKey &startKey) : btree(btree), count(0), pos(0)
        {
            fill(startKey);
        }

        void fill(const DPKey &nextKey)
        {
        ...
        bool operator!=(const range_iterator &rhs) = delete;
        DPKey key() { return kvs[pos].first; }
        Value value() { return kvs[pos].second; }
    };

    range_iterator lookup_range(const DPKey &startKey)
    {
    ...
    {
        BTreeLeaf<DPKey, Value> *node;
        int pos;
        explicit unsafe_iterator() : node(nullptr), pos(-1) {}
        explicit unsafe_iterator(BTreeLeaf<DPKey, Value> *node, int pos = 0) : node(node), pos(pos) {}
        ...
        DPKey key() { return node->data[pos].first; }
    ...
    unsafe_iterator begin_unsafe()
    {
        ...
        return unsafe_iterator((BTreeLeaf<DPKey, Value>*)node, 0);
    }
