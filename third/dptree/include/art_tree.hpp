...
std::function<void (uintptr_t, uint8_t[])> loadKey = [](uintptr_t tid,uint8_t key[]) {
...
thread_local std::vector<Node*> ancestors;
			...
			if (node->prefixLength <= maxPrefixLength) {
				...
				if (pos < node->prefixLength) {
					if (node->prefix[pos] < key[depth + pos]) {
						goto backtrack;
					} else { // node->prefix[pos] > key[depth + pos]
						return minimum(node);
					}
				}
			} else {
				...
				if (pos >= maxPrefixLength) {
					...
					if (pos < node->prefixLength) {
						if (leafKey[depth + pos] < key[depth + pos]) {
							goto backtrack;
						} else { // leafKey[depth + pos] > key[depth + pos]
                            pref = true;
							return minLeaf;
						}
					}
...
void destroyNode(Node* node) {
    ...
    switch (node->type) {
      case NodeType4: destroyNode4(static_cast<Node4*>(node)); break;
      case NodeType16: destroyNode16(static_cast<Node16*>(node)); break;
      case NodeType48: destroyNode48(static_cast<Node48*>(node)); break;
      case NodeType256: destroyNode256(static_cast<Node256*>(node)); break;
    }
}
...
Node* bulkLoad(std::vector<std::pair<DPKey, uintptr_t>> & kvs, int lo, int hi, int depth, KeyByteExtractFunc ExtractKeyByte, KeyLenFunc KeyLen) {
	if (hi - lo + 1 == 1) return makeLeaf(kvs[lo].second);
	int childCnt;
	int singlePart;
	...
	for (int i = 0; i < 256; ++i) {
		uint8_t byte = i;
		if (parts[byte].first == -1) continue;
