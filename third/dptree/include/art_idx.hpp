...
struct art_tree
{
    ...
    static art_tree* bulkLoadCreate(std::function<void(uintptr_t, uint8_t[])> loadKey, std::function<void(uintptr_t, uint8_t[])> loadLowerBoundKey, std::vector<std::pair<DPKey, uintptr_t>> &kvs, int lo, int hi, int depth, KeyByteExtractFunc ExtractKeyByte, KeyLenFunc KeyLen) {
    ...
    template <class DPKey,
              class KeyLenFunc,
              class KeyByteExtractFunc>
    static Node *bulkLoad(std::vector<std::pair<DPKey, uintptr_t>> &kvs, int lo, int hi, int depth, KeyByteExtractFunc ExtractKeyByte, KeyLenFunc KeyLen)
    {
        if (hi - lo + 1 == 1)
