...
namespace ART_OLC {
    ...
    TID Tree::lookup(const DPKey &k, ThreadInfo &threadEpocheInfo) const {
    ...
    bool Tree::lookupRange(const DPKey &start, const DPKey &end, DPKey &continueKey, TID result[],
    ...
    TID Tree::checkKey(const TID tid, const DPKey &k) const {
        DPKey kt;
        this->loadKey(tid, kt);
        if (k == kt) {
            return tid;
    ...
    void Tree::insert(const DPKey &k, TID tid, ThreadInfo &epocheInfo, std::function<void()> insert_func) {
    ...
    void Tree::remove(const DPKey &k, TID tid, ThreadInfo &threadInfo) {
    ...
    inline typename Tree::CheckPrefixResult Tree::checkPrefix(N *n, const DPKey &k, uint32_t &level) {
        if (n->hasPrefix()) {
    ...
    typename Tree::CheckPrefixPessimisticResult Tree::checkPrefixPessimistic(N *n, const DPKey &k, uint32_t &level,
                                                                        uint8_t &nonMatchingKey,
    ...
    typename Tree::PCCompareResults Tree::checkPrefixCompare(const N *n, const DPKey &k, uint8_t fillKey, uint32_t &level,
                                                        LoadKeyFunction loadKey, bool &needRestart) {
        if (n->hasPrefix()) {
            DPKey kt;
            for (uint32_t i = 0; i < n->getPrefixLength(); ++i) {
    ...
    typename Tree::PCEqualsResults Tree::checkPrefixEquals(const N *n, uint32_t &level, const DPKey &start, const DPKey &end,
                                                      LoadKeyFunction loadKey, bool &needRestart) {
        if (n->hasPrefix()) {
            DPKey kt;
            for (uint32_t i = 0; i < n->getPrefixLength(); ++i) {
