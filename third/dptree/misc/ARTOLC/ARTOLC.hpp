...
class ArtOLCIndex
{
  public:
    ...
    static void setKey(DPKey &k, uint64_t key) { k.setInt(key); }
    static void setKey(DPKey &k, GenericKey<31> key) { k.set(key.data, 31); }
    ...
    ArtOLCIndex(uint64_t kt)
    {
        if (sizeof(KeyType) == 8)
        {
            idx = new ART_OLC::Tree([](TID tid, DPKey &key) {
