...
static uint64_t FF_CACHE_LINE_SIZE = 64;
#define QUERY_NUM 25

#define FF_IS_FORWARD(c) (c % 2 == 0)
...
static inline void clflush(char *data, int len) {
  volatile char *ptr = (char *)((unsigned long)data & ~(FF_CACHE_LINE_SIZE - 1));
  mfence();
  for (; ptr < data + len; ptr += FF_CACHE_LINE_SIZE) {
#ifdef CLFLUSH
...
public:
  ...
  void *operator new(size_t size) {
    ...
    auto ret = index_pmem_alloc(size);
    return ret;
  }
...
const int cardinality = (FF_PAGESIZE - sizeof(header)) / sizeof(entry);
const int count_in_line = FF_CACHE_LINE_SIZE / sizeof(entry);
...
public:
  ...
  inline int count() __attribute__((optimize(0))) {
    ...
    do {
      ...
      while (count >= 0 && records[count].ptr != NULL) {
        if (FF_IS_FORWARD(previous_switch_counter))
          ++count;
        else
          --count;
      }
    ...
    } while (FF_IS_FORWARD(previous_switch_counter) !=
             FF_IS_FORWARD(hdr.switch_counter));
  ...
  inline bool remove_key(uint64_t key) __attribute__((optimize(0))) {
    ...
    if (FF_IS_FORWARD(hdr.switch_counter))
      ++hdr.switch_counter;
    else
    ...
    for (i = 0; records[i].ptr != NULL; ++i) {
      ...
      if (shift) {
        ...
        int remainder = records_ptr % FF_CACHE_LINE_SIZE;
        bool do_flush =
            (remainder == 0) ||
            ((((int)(remainder + sizeof(entry)) / FF_CACHE_LINE_SIZE) == 1) &&
             ((remainder + sizeof(entry)) % FF_CACHE_LINE_SIZE) != 0);
        if (do_flush) {
          clflush((char *)records_ptr, FF_CACHE_LINE_SIZE);
        }
      }
    }
  ...
  inline void insert_key(uint64_t key, char *ptr, int *num_entries,
    ...
    if (*num_entries == 0) { // this page is empty
      ...
      if (flush) {
        clflush((char *)this, FF_CACHE_LINE_SIZE);
      }
    } else {
      int i = *num_entries - 1, inserted = 0, to_flush_cnt = 0;
      records[*num_entries + 1].ptr = records[*num_entries].ptr;
      if (flush) {
        if ((uint64_t) & (records[*num_entries + 1].ptr) % FF_CACHE_LINE_SIZE == 0)
    ...
    if (hdr.sibling_ptr && (hdr.sibling_ptr != invalid_sibling)) {
      ...
      if (hdr.level > 0) {
        if (key >= hdr.sibling_ptr->hdr.highest.ikey) {
          ...
          return hdr.sibling_ptr->store(bt, left, key, right, true, with_lock,
                                        invalid_sibling);
        }
      } else {
        if (key >= hdr.sibling_ptr->hdr.highest.ikey) {
    ...
    if (num_entries < cardinality - 1) {
      ...
      if (hdr.leftmost_ptr == NULL) { // leaf node
        for (int i = m; i < num_entries; ++i) {
          sibling->insert_key(records[i].key.ikey, records[i].ptr, &sibling_cnt,
                              false);
        }
      } else { // internal node
  ...
  page *store(btree *bt, char *left, key_item *key, char *right, bool flush,
    ...
    if (hdr.sibling_ptr && (hdr.sibling_ptr != invalid_sibling)) {
      ...
      if (hdr.level > 0) {
        if (memcmp(key->key, hdr.sibling_ptr->hdr.highest.skey->key,
                   std::min(key->key_len,
                            hdr.sibling_ptr->hdr.highest.skey->key_len)) >= 0) {
          if (with_lock) {
            hdr.mtx->unlock(); // Unlock the write lock
          }
          return hdr.sibling_ptr->store(bt, left, key, right, true, with_lock,
    ...
    for (int i = 0; i < num_entries; ++i) {
      ...
      auto ok = records[i].key.skey;
      if (ok->key_len == key->key_len &&
          strncmp(ok->key, key->key, ok->key_len) == 0) {
        records[i].ptr = right;

        clflush((char *)&(records[i].ptr), sizeof(records[i].ptr));
    ...
    if (num_entries < cardinality - 1) {
      ...
      if (hdr.leftmost_ptr == NULL) { // leaf node
        for (int i = m; i < num_entries; ++i) {
          sibling->insert_key(records[i].key.skey, records[i].ptr, &sibling_cnt,
                              false);
        }
      } else { // internal node
        for (int i = m + 1; i < num_entries; ++i) {
      ...
      do {
        ...
        if (FF_IS_FORWARD(previous_switch_counter)) {
          ...
          if (memcmp(tmp_key->key, min->key,
            ...
            // if(memcmp(tmp_key->key, max->key, std::min(tmp_key->key_len,
            // max->key_len)) < 0 && off < num) {
            if (off < num) {
              if ((tmp_ptr = current->records[0].ptr) != NULL) {
                if (memcmp(tmp_key->key, current->records[0].key.skey->key,
                           std::min(tmp_key->key_len,
                                    current->records[0].key.skey->key_len)) ==
                    0) {
                  if (tmp_ptr) {
                    buf[off++] = (unsigned long)tmp_ptr;
                  }
              ...
              if (off < num) {
                if ((tmp_ptr = current->records[i].ptr) !=
                  ...
                  if (memcmp(tmp_key->key, current->records[i].key.skey->key,
                             std::min(tmp_key->key_len,
                                      current->records[i].key.skey->key_len)) ==
                      0) {
                    if (tmp_ptr) {
                      buf[off++] = (unsigned long)tmp_ptr;
                    }
  ...
  char *linear_search(btree *bt, uint64_t key) __attribute__((optimize(0))) {
    ...
    if (hdr.leftmost_ptr == NULL) { // Search a leaf node
      ...
      } while (FF_IS_FORWARD(hdr.switch_counter) !=
               FF_IS_FORWARD(previous_switch_counter));
    ...
    if (hdr.leftmost_ptr == NULL) { // Search a leaf node
      do {
        ...
        if (FF_IS_FORWARD(previous_switch_counter)) {
          ...
          if (memcmp(k->key, key->key, std::min(k->key_len, key->key_len)) ==
              0) {
            if ((t = records[0].ptr) != NULL) {
              if (memcmp(k->key, records[0].key.skey->key,
                         std::min(k->key_len, records[0].key.skey->key_len)) ==
                  0) {
                ret = t;
                continue;
              }
      ...
      do {
        ...
        if (FF_IS_FORWARD(previous_switch_counter)) {
          k = records[0].key.skey;
          if (memcmp(key->key, k->key, std::min(key->key_len, k->key_len)) <
              0) {
            if ((t = (char *)hdr.leftmost_ptr) != records[0].ptr) {
              ret = t;
              continue;
...
key_item *btree::make_key_item(char *key, size_t key_len, bool flush) {
  ...
  if (!flush) {
    auto res = posix_memalign(&aligned_alloc, 64, sizeof(key_item) + key_len);
    (void)res;
  } else {
    aligned_alloc = index_pmem_alloc(sizeof(key_item) + key_len);
  }
