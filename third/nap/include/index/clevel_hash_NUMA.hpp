...
GetPtr(T *p)
{
	return (T *)(((uint64_t)p) & 0x0000FFFFFFFFFFFC);
}
...
public:
	...
	using KV_entry_ptr_t = value_type *;

	using level_ptr_t = level_bucket *;

	using level_meta_ptr_t = level_meta *;
	...
	union KV_entry_ptr_u {
		...
		KV_entry_ptr_u(KV_entry_ptr_t off) : p(off)
		{
		}
	};
	...
	clevel_hash()
	    : meta((level_meta_ptr_t)pmemobj_direct(
		      make_persistent<level_meta>().raw())),
	      thread_num(0)
	{
		...
		tmp->up = nullptr;
		m->first_level = (level_ptr_t)pmemobj_direct(tmp.raw());
		...
		m->last_level = (level_ptr_t)pmemobj_direct(tmp.raw());
		...
		KV_entry_ptr_t e = get_entry(meta->first_level, 0, 0);
		if (e != nullptr) {
			// never fires.
			get_key(e);
		}
				   ...
				   const void *param)
	{
		...
		internal::make_persistent_object<value_type>(
			pop_numa[numa_map[global_thread_id]], KV_ptr, *v);
	}
				   ...
				   const void *param)
	{
		...
		// printf("wq\n");
		internal::make_persistent_object<value_type>(
			pop_numa[numa_map[global_thread_id]], KV_ptr,
			std::move(*const_cast<value_type *>(v)));
		// printf("qw\n");
	}
	...
	set_thread_num(size_type num)
	{
		if (thread_num > 0) {
			...
			for (size_type i = 0; i < thread_num; i++) {
				...
				if (tmp_level[di] != nullptr)
					delete_persistent<level_meta>(
						tmp_meta[di]);
				if (tmp_level[di] != nullptr)
					delete_persistent<level_bucket>(
						tmp_level[di]);
		...
		tmp_meta = make_persistent<persistent_ptr<level_meta>[]>(
			thread_num);
		tmp_level = make_persistent<persistent_ptr<level_bucket>[]>(
			thread_num);
		...
		do {
			...
			for (size_type j = 0; j < assoc_num; j++) {
				if (f_b.slots[j].x.partial == partial &&
				    f_b.slots[j].p != 0) {
					if (key_equal{}(GetPtr(f_b.slots[j].p)
								->first,
							key)) {
			...
			for (size_type j = 0; j < assoc_num; j++) {
				if (s_b.slots[j].x.partial == partial &&
				    s_b.slots[j].p != 0) {
					if (key_equal{}(GetPtr(s_b.slots[j].p)
								->first,
							key)) {
							...
							KV_entry_ptr_t e2)
{
	...
	if (tmp1_u.x.partial == tmp2_u.x.partial) {
		...
		if (e1 == e2) {
			if (CAS(&p2->p, e2, 0)) {
				pop.persist(&(p2->p), sizeof(uint64_t));
			}
		}

		// 2. Refer to different locations with the same contents
		else if (key_equal{}(e1->first, e1->first)) {
			if (CAS(&(p2->p), e2, 0)) {
				pop.persist(&(p2->p), sizeof(uint64_t));
	...
	while (true) {
		...
		*e = nullptr;
		...
		for (size_type i = n_levels - 1; i < n_levels; i--) {
			...
			bucket &f_b = cl->buckets[f_idx];
			for (size_type j = 0; j < assoc_num; j++) {
				if (!found_empty_in_b && f_b.slots[j].p == 0) {
			...
			bucket &s_b = cl->buckets[s_idx];
			for (size_type j = 0; j < assoc_num; j++) {
				if (!found_empty_in_b && s_b.slots[j].p == 0) {
					found_empty_in_b = true;
		...
		if (m_copy == meta) {
			...
			m_copy = meta;
			pop.persist(&meta, sizeof(uint64_t));
		}
	}
}
		...
		*e = nullptr;
		...
		for (size_type i = 0; i < n_levels; i++) {
				...
				if (f_e == 0) {
					...
					if (result != FOUND_IN_LEFT &&
						...
						*e = &(f_b.slots[j].p);
						...
						// printf("FIND %d %d %p\n", j,
						// assoc_num, f_b.slots[j].p);
					}
					continue;
				}

				if (f_b.slots[j].x.partial != partial ||
				    !key_equal{}(GetPtr(f_e)->first, key))
					continue;
			...
			for (size_type j = 0; j < assoc_num; j++) {
				...
				if (s_b.slots[j].x.partial != partial ||
				    !key_equal{}(GetPtr(s_e)->first, key))
					continue;
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::generic_insert(
	...
	void (*allocate_KV)(pool_base &, persistent_ptr<value_type> &,
			    const void *),
	size_type thread_id, size_type idss)
{
	...
	persistent_ptr<value_type> p_kv;
	allocate_KV(pop, p_kv, param);
	
	KV_entry_ptr_u created((KV_entry_ptr_t)pmemobj_direct(p_kv.raw()));
	...
	RETRY_INSERT:
		...
		if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT) {
			delete_persistent_atomic<value_type>(p_kv);
			...
			// printf("%p %p %p\n", e, old_e, created.p);
			// sleep(1);
			if (CAS(e, old_e, created.p)) {
				if (!m->is_resizing && meta->is_resizing &&
				    level_num == 0) {
						      ...
						      size_type thread_id)
{
	...
	while (true) {
		...
		do {
			...
			bucket &f_b = cl->buckets[f_idx];
			for (size_type j = 0; j < assoc_num; j++) {
				KV_entry_ptr_u tmp(f_b.slots[j].p);
				if (tmp.x.partial == partial && tmp.p != 0) {
					if (key_equal{}(GetPtr(tmp.p)->first,
							key)) {
						if (CAS(&(f_b.slots[j].p),
							tmp.p, 0)) {
							pop.persist(
								&(f_b.slots[j]
									  .p),
			...
			bucket &s_b = cl->buckets[s_idx];
			for (size_type j = 0; j < assoc_num; j++) {
				KV_entry_ptr_u tmp(s_b.slots[j].p);
				if (tmp.x.partial == partial && tmp.p != 0) {
					if (key_equal{}(GetPtr(tmp.p)->first,
							key)) {
						if (CAS(&(s_b.slots[j].p),
							tmp.p, 0)) {
							pop.persist(
								&(s_b.slots[j]
									  .p),
	...
	size_type thread_id)
{
	...
	while (true) {
		...
		KV_entry_ptr_t *e, old_e = nullptr;
						       ...
						       level_meta_ptr_t m_copy)
{
	...
	auto &pp = pop_numa[numa_map[global_thread_id]];
	persistent_ptr<level_bucket> p_bucket;
	persistent_ptr<level_meta> p_meta;
	if (cl->up == nullptr) {

		make_persistent_atomic<level_bucket>(pp, p_bucket);
		size_type new_capacity = cl->capacity * 2;
		std::cout << "Thread-" << thread_id << " starts expanding for "
			  << new_capacity << " buckets" << std::endl;

		make_persistent_atomic<bucket[]>(pp, p_bucket->buckets,
						 new_capacity);

		pp.persist(p_bucket->buckets);
		p_bucket->capacity = new_capacity;
		pp.persist(p_bucket->capacity);
		p_bucket->up = nullptr;
		pp.persist(&(p_bucket->up), sizeof(uint64_t));

		// Append a new level.
		bool rc = CAS(&(cl->up), 0,
			      (level_ptr_t)pmemobj_direct(p_bucket.raw()));

		if (rc == false) {
			// Ohter threads finished expanding
			pop.persist(&(cl->up), sizeof(uint64_t));

			delete_persistent_atomic<bucket[]>(p_bucket->buckets,
							   new_capacity);

			delete_persistent_atomic<level_bucket>(p_bucket);
		}
		...
		while (true) {
			...
			if (cl->capacity >= new_capacity) {
				...
				make_persistent_atomic<level_meta>(
					pp, p_meta, m->first_level,
					m->last_level, true);
			} else {
				assert(cl->up != nullptr);
				make_persistent_atomic<level_meta>(
					pp, p_meta, cl->up, m->last_level,
					true);
			}

			if (CAS(&meta, m_copy,
				(level_meta_ptr_t)pmemobj_direct(
					p_meta.raw()))) {
				pop.persist(&(meta), sizeof(uint64_t));
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::get_entry(level_ptr_t level,
							  ...
							  uint64_t slot_idx)
{
	return level->buckets[idx].slots[slot_idx].p;
}
