...
public:
  ...
  using kv_ptr_t = value_type *;
  using bucket_ptr_t = bucket_s *;
  using clht_hashtable_ptr_t = clht_hashtable_s *;
  using ht_ts_ptr_t = ht_ts *;
  ...
  struct ret {
    ...
    ret(bool _found)
        : found(_found), bucket_idx(0), step(0), slot_idx(0), expanded(false),
          capacity(0) {}

    ret()
        : found(false), bucket_idx(0), step(0), slot_idx(0), expanded(false),
          capacity(0) {}
  };
  ...
  clht(uint64_t n_buckets) {
    ...
    ht = (clht_hashtable_s *)pmemobj_direct(ht_tmp.raw());
  ...
  bucket_ptr_t clht_bucket_create_stats(pool_base &pop,
                                        clht_hashtable_s *ht_ptr, int &resize) {
    persistent_ptr<bucket_s> tmp;
    make_persistent_atomic<bucket_s>(pop_numa[numa_map[global_thread_id]], tmp);
    if (__sync_add_and_fetch(&ht_ptr->num_expands, 1) >=
        ht_ptr->num_expands_threshold)
      resize = 1;
    return (bucket_ptr_t)pmemobj_direct(tmp.raw());
  }
    ...
    do {
      for (size_t j = 0; j < ENTRIES_PER_BUCKET; j++) {
        if (bucket->slots[j] != nullptr &&
            key_equal{}(bucket->slots[j]->first, key))
  ...
  ret put(const value_type &value, size_type id) {
    return generic_insert(value.first, &value, allocate_KV_copy_construct, id);
  }

  ret put(value_type &&value, size_type id) {
    return generic_insert(value.first, &value, allocate_KV_move_construct, id);
  }
  ...
  ret generic_insert(const key_type &key, const void *param,
    ...
    do {
      for (size_t j = 0; j < ENTRIES_PER_BUCKET; j++) {
        if (bucket->slots[j] != nullptr &&
            key_equal{}(bucket->slots[j]->first, key)) {
          bucket->slots[j] = (kv_ptr_t)pmemobj_direct(tmp_entry.raw());
          pop.persist(&bucket->slots[j], sizeof(kv_ptr_t));
          unlock(pop, lock);
          return ret(true);
      ...
      if (likely(bucket->next == nullptr)) {
        if (unlikely(empty == nullptr)) {
          bucket_ptr_t b_new = clht_bucket_create_stats(pop, ht_ptr, resize);
          kv_ptr_t &s_new = b_new->slots[0];

          s_new = (kv_ptr_t)pmemobj_direct(tmp_entry.raw());
          pop.persist(&s_new, sizeof(kv_ptr_t));
          bucket->next = b_new;
          pop.persist(&bucket->next, sizeof(bucket_ptr_t));
        } else {
          *empty = (kv_ptr_t)pmemobj_direct(tmp_entry.raw());
          pop.persist(empty, sizeof(kv_ptr_t));
        }
        ...
        if (unlikely(resize)) {
          ...
          if (ht_status(true /* is_increase */, false /* just_print */) == 0)
  ...
  size_type ht_status(bool is_increase, bool just_print) {
    ...
    if (just_print) {
      ...
      if (full_ratio > 0 && full_ratio < CLHT_PERC_FULL_HALVE) {
        ...
        int ret = ht_resize_pes(true /* is_increase */, (size_t)inc_by_pow2);
        // return if crashed
        if (ret == -1)
          return 0;
      }
  ...
  int ht_resize_pes(bool is_increase, size_t by) {
...
#if CLHT_HELP_RESIZE == 1
    ht_old->table_tmp = ht_tmp.get();
    pop.persist(&ht_old->table_tmp, sizeof(clht_hashtable_ptr_t));

    for (difference_type idx = 0; idx < (difference_type)ht_old->num_buckets;
         idx++) {
    ...
    ht = ht_tmp.get();
    pop.persist(&ht, sizeof(clht_hashtable_ptr_t));
    ht_old->table_new = ht_tmp.get();
    pop.persist(&ht_old->table_new, sizeof(clht_hashtable_ptr_t));
  ...
  int bucket_cpy(bucket_s *bucket, clht_hashtable_s *ht_new) {
    ...
    do {
      for (size_t j = 0; j < ENTRIES_PER_BUCKET; j++) {
        if (bucket->slots[j] != nullptr) {
          hv_type hv = hasher{}(bucket->slots[j]->first);
          difference_type idx = static_cast<difference_type>(
  ...
  bool put_seq(clht_hashtable_s *hashtable, kv_ptr_t slot,
    ...
    do {
      ...
      if (bucket->next == NULL) {
        int null;
        bucket_ptr_t b_new = clht_bucket_create_stats(pop, hashtable, null);
        kv_ptr_t &s_new = b_new->slots[0];
        s_new = slot;
        pop.persist(&s_new, sizeof(kv_ptr_t));
        return true;
  ...
  void ht_resize_help(clht_hashtable_s *hashtable) {
    ...
    for (difference_type idx = (difference_type)hashtable->hash; idx >= 0;
         idx--) {
      bucket_s *bucket = &hashtable->table[idx];
      if (!bucket_cpy(bucket, hashtable->table_tmp))
        break;
    }
