...
public:
  ...
  level_hash(size_type level_power, size_type n_threads)
    ...
    for (size_t i = 0; i < 2; i++) {
      ...
      //      idx < (difference_type)levels[i]->capacity;
      //      idx++)
  ...
  ret generic_insert(const key_type &key, const void *param,
    ...
    kv_ptr_t created = (kv_ptr_t)pmemobj_direct(tmp_entry.raw());
    ...
    while (true) {
      ...
      for (size_t i = 0; i < 2; i++) {
        ...
        for (size_t j = 0; j < assoc_num; j++) {
          scoped_t slot_guard(levels[i]->buckets[f_idx].mutexes[j], true);
          if (levels[i]->buckets[f_idx].tokens[j] == 0) {
            insert_empty_slot(pop, i, f_idx, j, created);
            return ret(expanded, initial_capacity);
          }
          slot_guard.release();
          slot_guard.acquire(levels[i]->buckets[s_idx].mutexes[j], true);
  ...
  ret query(const key_type &key, size_type thread_id) {
    ...
    for (size_type i = 0; i < 2; i++) {
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        scoped_t slot_guard(levels[i]->buckets[f_idx].mutexes[j], false);
        if (levels[i]->buckets[f_idx].tokens[j] == 1 &&
            key_equal{}(levels[i]->buckets[f_idx].slots[j]->first, key)) {
          return ret(i, f_idx, j);
        }
      }

      for (size_type j = 0; j < assoc_num; j++) {
        scoped_t slot_guard(levels[i]->buckets[s_idx].mutexes[j], false);
        if (levels[i]->buckets[s_idx].tokens[j] == 1 &&
            key_equal{}(levels[i]->buckets[s_idx].slots[j]->first, key)) {
  ...
  ret erase(const key_type &key, size_type thread_id) {
    ...
    for (size_type i = 0; i < 2; i++) {
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        scoped_t slot_guard(levels[i]->buckets[s_idx].mutexes[j], true);
        if (levels[i]->buckets[s_idx].tokens[j] == 1 &&
            key_equal{}(levels[i]->buckets[s_idx].slots[j]->first, key)) {
    ...
    for (size_type i = 0; i < 2; i++) {
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        ...
        if (levels[i]->buckets[f_idx].tokens[j] == 1 &&
          ...
          pop.persist(&levels[i]->buckets[f_idx].slots[j], sizeof(kv_ptr_t));
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        ...
        if (levels[i]->buckets[s_idx].tokens[j] == 1 &&
          ...
          pop.persist(&levels[i]->buckets[s_idx].slots[j], sizeof(kv_ptr_t));
    ...
    for (difference_type old_idx = 0;
      ...
      for (size_type i = 0; i < assoc_num; i++) {
        if (levels[1]->buckets[old_idx].tokens[i] == 1) {
          const key_type &key = levels[1]->buckets[old_idx].slots[i]->first;
          ...
          for (size_type j = 0; j < assoc_num; j++) {
            if (interim_level->buckets[f_idx].tokens[j] == 0) {
              insert_empty_slot(pop, 2, f_idx, j,
            ...
            if (interim_level->buckets[s_idx].tokens[j] == 0) {
              insert_empty_slot(pop, 2, s_idx, j,
                                levels[1]->buckets[old_idx].slots[i]);
              succ = true;
              break;
            }
          ...
          if (!succ) {
            ...
            std::cerr << "The expanding succ" << std::endl;
          }
  ...
  void insert_empty_slot(pool_base &pop, size_type level_idx,
    ...
    b->slots[slot_idx] = e;
    pop.persist(&b->slots[slot_idx], sizeof(kv_ptr_t));
    b->tokens[slot_idx] = 1;
  ...
  bool try_movement(pool_base &pop, size_type level_idx,
    ...
    for (size_type i = 0; i < assoc_num; i++) {
      scoped_t src_guard(levels[level_idx]->buckets[bucket_idx].mutexes[i],
                         true);
      const key_type &m_key =
          levels[level_idx]->buckets[bucket_idx].slots[i]->first;
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        scoped_t dst_guard(levels[level_idx]->buckets[jdx].mutexes[j], true);
        if (levels[level_idx]->buckets[jdx].tokens[j] == 0) {
          ...
          pop.persist(&levels[level_idx]->buckets[bucket_idx].tokens[i],
                      sizeof(uint8_t));
  ...
  bool b2t_movement(pool_base &pop, difference_type bucket_idx, kv_ptr_t e) {
    for (size_type i = 0; i < assoc_num; i++) {
      scoped_t src_guard(levels[1]->buckets[bucket_idx].mutexes[i], true);
      assert(levels[1]->buckets[bucket_idx].tokens[i] == 0);

      const key_type &m_key = levels[1]->buckets[bucket_idx].slots[i]->first;
      hv_type f_hv = first_hash(m_key);
      hv_type s_hv = second_hash(m_key);
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        scoped_t dst_guard(levels[0]->buckets[f_idx].mutexes[j], true);
        ...
        dst_guard.release();
        dst_guard.acquire(levels[0]->buckets[s_idx].mutexes[j]);
