...
#define STORE_ORDER std::memory_order_release
    ...
    public:
        ...
        public:
#ifdef CCEH_2_ALEXOL
            bool bulk_load(const std::pair<K, V> data[], int num_keys);

            bool put_pm_only(const K &key, const V &value, KVOffset *offset);
#endif
...
#ifdef CCEH_2_ALEXOL
            bool put_pm_only(const K &key, const V &value, KVOffset *offset, bool delete_old);
#endif
...
#ifdef CCEH_2_ALEXOL
        alexol::Alex<K, KVOffset, alexol::AlexCompare, std::allocator<std::pair<K, KVOffset>>, false> *map_;
#else
        cceh::CCEH<K> map_;
#endif
    ...
    template <typename K, typename V>
    Viper<K, V>::Viper(ViperBase v_base, const std::filesystem::path pool_dir, const bool owns_pool, const ViperConfig v_config) : v_base_{v_base},
#ifdef CCEH_2_ALEXOL
    // map_{},
#else
                                                                                                                                   map_{131072},
#endif
                                                                                                                                   owns_pool_{owns_pool}, v_config_{v_config}, pool_dir_{pool_dir},
...
#ifdef CCEH_2_ALEXOL
        map_ = new alexol::Alex<K, KVOffset>();
#endif
    ...
    ViperInitData init_dram_pool(uint64_t pool_size, ViperConfig v_config, const size_t block_size)
    {
        ...
        ViperFileMetadata v_metadata{.block_offset = PAGE_SIZE, .block_size = block_size, .alloc_size = alloc_size, .num_used_blocks = 0, .num_allocated_blocks = 0, .total_mapped_size = pool_size};
                                 ...
                                 bool is_new_pool, ViperConfig v_config, const size_t block_size)
    {
        ...
        if (is_new_pool)
        {
            ...
            MMAP_CHECK(metadata_addr)
            ViperFileMetadata v_metadata{.block_offset = PAGE_SIZE, .block_size = block_size, .alloc_size = alloc_size, .num_used_blocks = 0, .num_allocated_blocks = num_allocated_blocks, .total_mapped_size = pool_size};
                                     ...
                                     bool is_new_pool, ViperConfig v_config)
    {
        ...
        return ViperBase{.file_descriptor = init_data.fd, .is_new_db = is_new_pool, .is_file_based = is_file_based, .v_metadata = init_data.meta, .v_mappings = std::move(init_data.mappings)};
    }
...
#ifdef CCEH_2_ALEXOL // FIXME:
        // // TODO: temporarily consider fixed size 8B key without fingerprinting
        const bool is_new_item = true;
...
#ifdef CCEH_2_ALEXOL
    template <typename K, typename V>
    bool Viper<K, V>::Client::put_pm_only(const K &key, const V &value, KVOffset *offset, const bool delete_old)
    {
        v_page_->lock();
        ...
        if (free_slot_idx >= free_slots->size())
        {
            ...
            v_page_->unlock();
            update_access_information();
            return put_pm_only(key, value, offset, delete_old);
        }
        ...
        // Save data for DRAM map bulkload.
        const bool is_new_item = true; // TODO:
        offset->block_number = v_block_number_;
        offset->page_number = v_page_number_;
        offset->data_offset = free_slot_idx;
    ...
    //     // Store data in DRAM map.
    // #ifdef CCEH_2_ALEXOL // FIXME:
    //     // this->viper_.map_->insert(key, var_offset);
    //     this->viper_.map_->insert( std::stoul(key), var_offset);
...
#ifdef CCEH_2_ALEXOL
    template <typename K, typename V>
    bool Viper<K, V>::Client::bulk_load(const std::pair<K, V> data[], int num_keys)
    {
        auto bulk_data = new std::pair<K, KVOffset>[num_keys];

        for (int i = 0; i < num_keys; i++)
        {
            bulk_data[i].first = data[i].first;
            put_pm_only(data[0].first, data[0].second, &bulk_data[i].second, false);
        }

        this->viper_.map_->bulk_load(bulk_data, num_keys);
        delete bulk_data;
        return true;
    }

    template <typename K, typename V>
    bool Viper<K, V>::Client::put_pm_only(const K &key, const V &value, KVOffset *offset)
    {
        return put_pm_only(key, value, offset, true);
    }
#endif
    ...
    bool Viper<K, V>::Client::get(const K &key, V *value)
    {
        ...
        {
#ifdef CCEH_2_ALEXOL
            KVOffset kv_offset = KVOffset::Tombstone();
            this->viper_.map_->get_payload(key, &kv_offset);
#else
    ...
    void Viper<K, V>::Client::free_occupied_slot(const KVOffset offset_to_delete, const K &key, const bool delete_offset)
    {
        ...
        if (v_block_number_ == block_number && v_page_number_ == page_number)
        {
            ...
            {
#ifdef CCEH_2_ALEXOL
                this->viper_.map_->insert(key, KVOffset::Tombstone());
                // this->viper_.map_->erase(key);// FIXME:
#else
        ...
        {
#ifdef CCEH_2_ALEXOL
            // this->viper_.map_->insert(key, IndexV::NONE());
            this->viper_.map_->insert(key, KVOffset::Tombstone());
            // this->viper_.map_->erase(key);// FIXME:
#else
            this->viper_.map_.Insert(key, IndexV::NONE(), key_check_fn);
    ...
    // #ifdef CCEH_2_ALEXOL
    //             IndexV offset = IndexV::Tombstone(); // FIXME: string type not considered
    //             // map_->get_payload(key, &offset);
    // #else
